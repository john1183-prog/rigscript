...
- **Fullscreen Script Editor destination (`ScriptEditorScreen`) & Canvas layout normalization:**
  ...
  - **Dedicated destination**: Added navigation route `editor/{projectId}/script?time={time}` with `ScriptEditorScreen`, mirroring `PoseEditorScreen`'s architecture. Contains ONLY: minimal top bar (back navigation icon, title, live parse status indicator), action row (Prompt, Import, Pose, Overlay menu at `currentTimeSec`), warnings/errors block, and `BasicTextField` filling all remaining screen space with scoped `imePadding`.
  - **Canvas height normalization**: Replaced the `if (selectedTab == 0) 0.14f else 0.38f` workaround with a uniform 0.38f across all tabs in `EditorScreen`. Re-enabled the preview aspect toggle across all tabs. Removed root `imePadding` from `EditorScreen`.
  - **Script tab entry point (Option A)**: `EditorScreen`'s Script tab now renders `ScriptSummaryPanel` with event/overlay counts, duration, live validation indicator, a monospace read-only preview card, and a prominent "Edit Script" button (tapping either the button or preview card navigates to the fullscreen editor, preserving the current playhead timestamp for overlay insertions).
  - **Single source of truth & state preservation**: `MainViewModel` remains the authoritative source of truth (`_scriptText`, `_scriptError`, `_scriptWarnings`, `_activeProject`). When navigating between `EditorScreen` and `ScriptEditorScreen`, `MainViewModel` preserves in-progress raw text and validation state across backstack pushes and pops with zero data loss and no second source of truth.
  - **Pose Library integration**: Opening the pose library from `ScriptEditorScreen` reuses the existing `POSES` composable wiring (`parentEntry.destination.route.startsWith("editor")`), inserting the chosen pose directly into `MainViewModel` and popping back into `ScriptEditorScreen`.

- **Appearance tab sub-categories reorganization & pinned Presets strip:**
  - **Problem & diagnosis**: `AppearancePanel` in `EditorScreen` previously rendered nine distinct sections sequentially in a single massive vertical `Column`: Presets, Colors, Character, Scene, Audio Response, Motion & Face, Reference Overlay, Background Music, and Sound Effects. This required excessive vertical scrolling to reach audio or reference controls, and made navigating between related visual settings cumbersome on mobile screens.
  - **Five-category grouping**:
    1. **Character**: Colors (figure, preview/export background) + Character & Face controls (scale, stroke width, joint radius, root anchor X/Y, show joints in preview/export, show grid, show mouth, mouth color, head size, neck length, show eyes, eye color, eye spacing, eye vertical position, eye aspect ratio, eyebrow color, eyebrow expression hint) + "Reset appearance to defaults" action.
    2. **Scene**: Background gradient (style toggle, bottom gradient color) + Ground line (toggle, color picker, Y-fraction position slider).
    3. **Motion**: Audio Response controls (enable amplitude motion, silence threshold, talk torso amplitude, head nod amplitude, talk frequency, smoothing, arm sway, idle breath amplitude) + Motion & Face controls (spring feel on every transition, natural idle blinking, idle fidget during pauses, fidget amplitude) + "Reset audio response to defaults" action.
    4. **Reference**: Reference Overlay controls (None / Image / Text selector, image picker/remover, 4-way crop sliders, text input, text color, backdrop switch, position X/Y, size slider, in-front-of-figure toggle).
    5. **Audio**: Background Music controls (music picker/remover, music volume, narration volume, loop toggle) + Sound Effects (clip list with ID rename, delete, volume slider, add sound effect picker, and bundled CC0 starter sound library chips with "Add all").
  - **Persistent pinned Presets strip**: `Presets` remains a persistent horizontal strip pinned above the `ScrollableTabRow` sub-category selector rather than becoming a sub-category. Because presets alter cross-cutting visual properties across colors, character geometry, and scene styling, keeping the preset strip permanently accessible allows users to apply or save presets from any sub-category without navigating away.
  - **Visual language & nested scrolling discipline**: Uses Material 3 `ScrollableTabRow` with matching `surface`/`primary` colors and `16.dp` edge padding, consistent with the top-level Script/Appearance/Export `TabRow`. Pinned `PresetsStrip` sits above the category selector, separated by a subtle divider. Only the selected sub-category's controls are rendered below the tab row inside an independent vertically scrollable container (`Modifier.fillMaxSize().verticalScroll(rememberScrollState()).padding(16.dp)`), eliminating nested vertical scrolling conflicts and preserving independent scroll positions per category.
  - **Zero behavioral or rendering changes**: All 18 callbacks, pickers, dialogs, defaults, ranges, and conditional visibility rules were preserved verbatim without touching underlying appearance data classes or rendering pipelines.

- **Consolidated draggable timeline (`ConsolidatedTimeline`) & intentional reversal of tap-only timeline interaction:**
  - **Problem & diagnosis**: `EditorScreen` previously rendered four separate stacked controls between `AudioBar` and the top `TabRow`: `AmplitudeWaveform`, `EventTimelineStrip`, `OverlayTimelineStrip`, and a separate scrubber `Slider`. These occupied separate vertical bands, required users to grab a small dedicated slider thumb rather than manipulating the timeline surface directly, and suffered from horizontal coordinate misalignment because Material 3 `Slider`'s internal thumb inset did not match the edge-to-edge `Canvas` strips above it.
  - **Explicit design reversal**: Earlier V2 decisions (`EventTimelineStrip` and `OverlayTimelineStrip`) deliberately restricted timeline strips to tap-only (`detectTapGestures`) navigation while relying on a separate `Slider` for scrubbing. This decision is **intentionally reversed**: consolidating the amplitude waveform, overlay layer spans, event markers, and playhead into a single direct-manipulation surface makes drag-to-scrub immediately discoverable, guarantees pixel-exact alignment across all layers, and eliminates the redundant scrubber `Slider`. (Note: event/overlay creation and property editing remain driven by the JSON script and toolbar actions rather than drag-to-edit-event handles).
  - **Shared `Canvas` & layering order**: `ConsolidatedTimeline` renders all timeline layers inside one shared `Canvas` using a unified `timeToX` / `xToTime` mapping over `[0, size.width]`:
    1. **Waveform / track baseline**: Subtle rounded track container + center baseline (ensuring visibility before audio is imported) + audio amplitude bars (`primary @ 0.45f`, up to 85% height).
    2. **Overlay spans**: Rounded horizontal bars in the upper band (`tertiary @ 0.82f`, spanning `[startSec, endSec)`).
    3. **Event markers**: Vertical ticks (`primary @ 0.90f`) topped with circular pips at each `ScriptEvent.timeSec`.
    4. **Playhead**: Full-height high-contrast vertical line + top handle cap at `scrubberPos`.
  - **Direct drag + tap-to-nearest snapping (`awaitEachGesture`)**: Keyed on `Unit` with `rememberUpdatedState` so `scrubberPos` updates never cancel an active drag. Continuous horizontal dragging maps pointer `x` directly to project time (`[0f, totalDuration]`) and invokes `seekPlayback`. A simple tap (within `touchSlop`) snaps to the nearest `ScriptEvent.timeSec` or `OverlayLayer.startSec` within an `18.dp` hit radius (preserving the existing `"pose @ 1.5s"` and `"id 1.0s–3.0s"` inspection labels), or seeks directly to the tapped timestamp when tapping open timeline space.
  - **Animated expand/collapse target**: Normally compact (`36.dp` canvas + `16.dp` clock/label header), expanding smoothly via `animateDpAsState` (`Spring.DampingRatioNoBouncy`) to `68.dp` immediately on touch-down/drag and settling back `1200ms` after pointer release without affecting horizontal coordinate mapping.
